...
- 대용량에서 벡터가 용량의 대부분 → int8 양자화 모델(`model_quantized.onnx`, `dtype:'q8'`)을 이미 사용 중(ADR 0016 번들 결정과 연계).
- 임베딩 모델은 번들로 제공되므로(ADR 0016), 오프스크린 최초 로드 시 네트워크 지연 없이 즉시 모델을 올릴 수 있다.
