...
> Paste each block into the matching field in the Web Store dashboard.
...
```
Find pages you've read by meaning, then ask about them. Your reading data stays on your device.
```
...
You can also ask a question about your saved pages. Recall retrieves the most relevant saved passages, then writes a short answer from those passages with sources.

Truly private - and it's not just a promise, it's how the code is built:
- Your reading data stays on your device: saved page text, URLs, search queries, and Ask questions are processed locally.
- Model files may be loaded from the extension package or from a configured Cloudflare R2 model bucket. Those files are public model artifacts, not your data.
...
```
Recall has a single purpose: to help you re-find and ask about web pages you have already read by saving their readable text locally, searching that text by meaning, and answering only from saved passages. Captured page data, search queries, and Ask questions stay on your own device.
```
